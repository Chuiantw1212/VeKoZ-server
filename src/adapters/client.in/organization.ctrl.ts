...
router.use(bearer())
    .get('/organization/list', async () => {
        ...
        const organizations: IOrganization[] = await OrganizationService.getDocList()
    ...
    .post('/organization', async ({ request, bearer }) => {
        ...
        await OrganizationService.mergeUniqueDoc(user.uid, newOrganization.id, newOrganization)
